// frontend/src/components/bulk-ops/BulkMoveDialog.tsx
import { useState } from 'react';
import { useBulkMove } from '../../hooks/useBulkOps';
import type { BulkMoveResult } from '../../api/bulkOps';
...
export function BulkMoveDialog({ schoolId, onClose }: Props) {
  const [fromDate, setFromDate] = useState('');
  const [toDate, setToDate] = useState('');
  const [offsetMinutes, setOffsetMinutes] = useState(60);
  const [eventType, setEventType] = useState<'all' | 'game' | 'practice'>('all');
  const [preview, setPreview] = useState<BulkMoveResult | null>(null);

  const mutation = useBulkMove(schoolId);

  const handlePreview = async () => {
    const result = await mutation.mutateAsync({
      fromDate, toDate, offsetMinutes, eventType, dryRun: true,
    });
    setPreview(result);
  };
  ...
  const handleExecute = async () => {
    await mutation.mutateAsync({
      fromDate, toDate, offsetMinutes, eventType, dryRun: false,
    });
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] overflow-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-800">Bulk Move Events</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={offsetMinutes} onChange={(e) => setOffsetMinutes(Number(e.target.value))}
                className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" />
              <p className="text-xs text-gray-500 mt-1">Positive = later, negative = earlier</p>
            </div>
            <div>
          ...
          {preview && (
            <div className="border border-gray-200 rounded-lg">
              <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 text-sm font-medium text-gray-700">
                Preview: {preview.count} event{preview.count !== 1 ? 's' : ''} will be moved
              </div>
                      ...
                      {preview.moves.map((m) => (
                        <tr key={m.id}>
                          ...
                          <td className="px-3 py-2 text-gray-500">
                            {new Date(m.originalDatetime).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                          </td>
                          <td className="px-3 py-2 text-blue-600 font-medium">
                            {new Date(m.newDatetime).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <div className="p-4 border-t border-gray-200 flex justify-end gap-2">
          ...
          {preview && preview.count > 0 && (
            <button onClick={handleExecute} disabled={mutation.isPending}
              ...
              Move {preview.count} Event{preview.count !== 1 ? 's' : ''}
            </button>
          )}
        </div>
      </div>
